...
def extract_debate_speech(game_logs: list, player_name: str) -> list[str]:
    collected = []
...
def main():
    ...
    for folder in sorted(os.listdir(args.base_dir)):
        ...
        speech = extract_debate_speech(game_logs, player)
        results[mode].extend(speech)
        counts[mode] += 1
        print(f"  {folder}: player={player}, mode={mode}, seed={seed}, "
              f"debate_entries={len(speech)}")

    print("\n=== SUMMARY ===")
    for m in MODES:
        print(f"  {m}: {counts[m]} sessions, {len(results[m])} speech entries total")

    out = f"debate_{args.model}_{args.role}.json"
