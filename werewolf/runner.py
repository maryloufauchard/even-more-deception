...
_MALICIOUS = flags.DEFINE_string(
    "malicious_player", None, "Name of the malicious player, None if normal"
)
_MODE_OBJECTIVE = flags.DEFINE_string(
    "primary_mode", "Benevolent", "Benevolent, Individualistic or Malevolent"
)
_MODE_SUBOBJECTIVE = flags.DEFINE_string(
    "secondary_mode", None, "Benevolent, Individualistic, Malevolent or None"
)
_SEED = flags.DEFINE_integer(
    "seed", 1, "Repetition Number"
)
_MODEL_NAME = flags.DEFINE_string(
    "model_name", "Unknown", "Model family"
)

DEFAULT_WEREWOLF_MODELS = ["flash", "pro1.5"]
DEFAULT_VILLAGER_MODELS = ["flash", "pro1.5"]
RESUME_DIRECTORIES = ['.../werewolf_arena/logs/session_20251220_135243']
...
def initialize_players(
    ...
    malicious_player = _MALICIOUS.value # value should be Doctor, Seer, Villager or Werewolf
    main_mode = _MODE_OBJECTIVE.value
    print(main_mode)
    sub_mode = _MODE_SUBOBJECTIVE.value
    print(sub_mode)

    for p in all_players:
        if p.role == malicious_player:
            print("Malicious player is: ")
            print(p.name)
            print(p.role)
            p.malicious = True 
            p.primary_mode = main_mode
            p.secondary_mode = sub_mode
            break
...
def resume_game(directory: str) -> bool:
    ...
    logging.save_adversary_logs(gm.adversary_logs, directory)  # NEW
...
def run_game(
    ...
    filename = f"session_{model_name}_{malicious_player}_{main_mode}_{sub_mode}_{repet}"
    print(filename)
    log_directory = logging.log_directory(filename)
    logging.save_game(state, gamemaster.logs, log_directory)
    print(f"Game logs saved to: {log_directory}")
    logging.save_adversary_logs(gamemaster.adversary_logs, log_directory) # NEW
    print(f"Adversary detection logs saved to: {log_directory}/adversary_detection.json")
...
def run() -> None:
    ...
    malicious_player = _MALICIOUS.value 
    if _RUN_GAME.value:
