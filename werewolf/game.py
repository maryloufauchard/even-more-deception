...
class GameMaster:
  ...
  def __init__(
    ...
    self.adversary_logs = [] # new--> adversary detection
  ...
  def eliminate(self):
    ...
    if self.current_round_num == 0: 
      # check that killed player is not malicious
      # 1. get malicious player name 
      malicious_name = None
      for player in self.state.players.values():
        if getattr(player, 'malicious', False):
            malicious_name = player.name
            print("checking who is malicious")
            print(malicious_name)
            break
      condition = False
      if eliminated == malicious_name: 
        print("Malicious player was killed")
        condition = True
      # 2. if it is malicious, run again! (does not affect any memory, but can happen technically infinitely with 0 probability)
      while condition:  
        eliminated, log = wolf.eliminate() 
        if eliminated == malicious_name:
          condition = True
          ...
          condition = False
      print("player eliminated first night is a ")
      print(eliminated)
  ...
  # NEW FUNCTION FOR DETECTING UNEXPECTED ADVERSARIES:
  def run_adversary_detection(self,phase:str):
    """Ask each remaining alive player if they believe there is an unexpected adversary"""
    tqdm.tqdm.write(f"Conducting adversary detection after {phase} phase...")
    adversary_suspicions = {}
    phase_log = []

    with ThreadPoolExecutor(max_workers=self.num_threads) as executor:
        player_suspicions = {
            player_name: executor.submit(
                self._get_adversary_suspicion, player_name
            )
            for player_name in self.this_round.players
        }

        for player_name, suspicion_task in player_suspicions.items():
            suspect, log = suspicion_task.result()

            adversary_suspicions[player_name] = suspect
            phase_log.append((player_name, log))

            if suspect:
                tqdm.tqdm.write(
                    f"{player_name} suspects {suspect} of unexpected behavior"
                )
            else:
                tqdm.tqdm.write(
                    f"{player_name} sees no unexpected adversaries"
                )

    self.adversary_logs.append({
        "round": self.current_round_num,
        "phase": phase,
        "debate_transcript": list(self.this_round.debate),
        "suspicions": adversary_suspicions,
        "logs": phase_log,
    })
    
    return adversary_suspicions
    
  # NEW FUNCTION (HELPER OF PREVIOUS ONE)
  def _get_adversary_suspicion(self, player_name: str):
    """Get adversary suspicion from a specific player."""
    # 1. Find if malicious player is Individualistic or Malevolent
    player = self.state.players[player_name]
    suspect, log = player.adversary()

    if (suspect is not None) and suspect not in self.state.players.keys():
      if suspect != "None":
        raise ValueError(
            f"{player_name} returned invalid adversary {suspect}"
        )
  ...
  def run_day_phase(self):
    ...
    MID_DEBATE_TURN = MAX_DEBATE_TURNS // 2 # 4 normally
    ...
    for idx in range(MAX_DEBATE_TURNS):
      ...
      if idx == MID_DEBATE_TURN -1: 
        tqdm.tqdm.write("Midpoint day check for adversary detection")
        self.run_adversary_detection("MID-DAY")
    
    tqdm.tqdm.write("End of debate/vote, running end of day check for adversary detection")
    self.run_adversary_detection("END-DAY")
