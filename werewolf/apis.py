...
def generate(model, **kwargs):
    ...
    elif "llama" in model or "meta-llama" in model or 'Llama' in model:
        return generate_openai(model, **kwargs)
    elif "Qwen" in model or "QwQ" in model:
        return generate_qwen_2(model, **kwargs)
    elif "google" in model or "gemma" in model:
        return generate_gemma(model, **kwargs)
    else:
...
def generate_openai(model: str, prompt: str, json_mode: bool = True, **kwargs):
    ...
    api_key = os.environ.get("OPENAI_API_KEY") or "local-token"
...
def generate_gemma(model:str, prompt: str, json_mode: bool= True, **kwargs):
    base_url = os.environ.get("OPENAI_BASE_URL")
    api_key = os.environ.get("OPENAI_API_KEY") or "local-token"
    client = OpenAI(api_key=api_key, base_url=base_url) if base_url else OpenAI(api_key=api_key)
    response_format = {"type": "text"}
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "Keep all string values under 2000 characters. "
                    "Do not use tabs or literal newlines inside JSON string values."
                ),
            },
...
def generate_qwen(model: str, prompt: str, json_mode: bool = True, **kwargs):
    base_url = os.environ.get("OPENAI_BASE_URL")
    api_key = os.environ.get("OPENAI_API_KEY") or "local-token"
    client = OpenAI(api_key=api_key, base_url=base_url) if base_url else OpenAI(api_key=api_key)
    ...
    return txt
def generate_qwen_2(model: str, prompt: str, json_mode: bool = True, **kwargs):
    base_url = os.environ.get("OPENAI_BASE_URL")
    api_key = os.environ.get("OPENAI_API_KEY") or "local-token"
    client = OpenAI(api_key=api_key, base_url=base_url) if base_url else OpenAI(api_key=api_key)
    response_format = {"type": "text"}
