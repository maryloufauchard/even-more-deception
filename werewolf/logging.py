...
def log_directory(filename) -> str:
    ...
    directory = f"{os.getcwd()}/logs_adversarial/{filename}"
    return directory
...
# NEW : for detection
def save_adversary_logs(adversary_logs: list, directory: str):
    
    os.makedirs(directory, exist_ok=True)
    adversary_file = f"{directory}/adversary_detection.json"

    serializable_logs = []
    for entry in adversary_logs:
        logs_serialized = []
        for player_name, lm_log in entry["logs"]:
            logs_serialized.append({
                "player": player_name,
                "prompt": lm_log.prompt,
                "raw_response": lm_log.raw_resp,
        ...
        serializable_logs.append({
            ...
            "detailed_logs": logs_serialized,
        })

    with open(adversary_file, "w") as file:
        json.dump(serializable_logs, file, indent=4)