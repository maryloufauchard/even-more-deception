...
class GameView:
  ...
  def __init__(
    ...
    self.all_player: List[str] = current_players.copy()
...
class Player(Deserializable):
  ...
  def _generate_action(
    ...
    result_key, allowed_values = (
        (action, options)
        if action in ["vote", "remove", "investigate", "protect", "bid","adversary"] # CHANGED
        else (None, None)
    )
  ...
  # NEW : EACH PLAYER CAN VOTE FOR UNEXPECTED ADVERSARY
  def adversary(self):
    """ Ask the player to identify suspicious behavior."""
    #possible_targets = [
    #    p for p in self.gamestate.current_players
    #    if p != self.name]
    possible_targets = self.gamestate.all_player
    if not possible_targets:
        return None, {"reasoning": "No other players to evaluate."}
    options = possible_targets + ["None"]
    suspect, log = self._generate_action(
        "adversary",
        options)
    if suspect == "None":
        suspect = None
  ...
  def from_json(cls, data: Dict[Any, Any]):
    ...
    primary_mode = data.get("primary_mode", "Benevolent")
    secondary_mode = data.get("secondary_mode", None)
    o = cls(name=name, role=role, model=model, malicious=malicious, primary_mode=primary_mode, secondary_mode=secondary_mode)
...
class Villager(Player):
  ...
  def __init__(
    ...
    super().__init__(
        name=name, role=VILLAGER, model=model, personality=personality, malicious=malicious, primary_mode=primary_mode, secondary_mode=secondary_mode
    )
  ...
  def from_json(cls, data: dict[Any, Any]):
    ...
    primary_mode = data.get("primary_mode", "Benevolent")
    secondary_mode = data.get("secondary_mode", None)
    o = cls(name=name, model=model, malicious=malicious, primary_mode=primary_mode, secondary_mode=secondary_mode)
...
class Werewolf(Player):
  ...
  def __init__(
    ...
    super().__init__(
        name=name, role=WEREWOLF, model=model, personality=personality, malicious=malicious, primary_mode=primary_mode, secondary_mode=secondary_mode
    )
...
class Seer(Player):
  ...
  def __init__(
    ...
    super().__init__(name=name, role=SEER, model=model, personality=personality, malicious=malicious, primary_mode=primary_mode, secondary_mode=secondary_mode)
...
class Doctor(Player):
  ...
  def __init__(
    ...
    super().__init__(
        name=name, role=DOCTOR, model=model, personality=personality, malicious=malicious, primary_mode=primary_mode, secondary_mode=secondary_mode
    )
